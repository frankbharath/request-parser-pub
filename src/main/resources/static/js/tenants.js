(function() {
	...
	app.controller("AddTenantCtrl",["constants", "resolvedProperties","TenantDataServiceFactory","$filter", "$state", function(constants, resolvedProperties, TenantDataServiceFactory, $filter, $state){
		var vm=this;
		...
		vm.moveinchange = function(){
			if(vm.moveindate===undefined){
				vm.moveoutmindate = new Date(
		    		...
		    		vm.moveinmindate.getMonth() + 1,
		    		vm.moveinmindate.getDate()
	  			);
			}else{
				vm.moveoutmindate = new Date(
		    		vm.moveindate.getFullYear(),
		    		vm.moveindate.getMonth() + 1,
		    		vm.moveindate.getDate()
	  			);
			}
		}
		vm.moveinchange();
